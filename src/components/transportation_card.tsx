...
import { useTransportationStore, useUserStore } from "../utils/store/app_store";
import { ActivitiesAPI } from "../utils/api";
import { addToast } from "@heroui/toast";

export default function TransportationCard({ transportation }: { transportation: any }) {
  const [activeTab, setActiveTab] = useState("notes");
  const navigate = useNavigate();
  const { user } = useUserStore();
  const { user_id } = useParams();
  const { populateTransportationForm, setIsUpdating, setCurrentTransportationId, setTransportationList, transportationList } = useTransportationStore();

  const handleEdit = () => {
    populateTransportationForm(transportation);
    setCurrentTransportationId(transportation.id);
    setIsUpdating(true);
    navigate(`/${user_id}/${transportation.overview_id}/new-transportation`);
  };

  const handleDelete = async (transportation_id: string) => {
    addToast({title: "Deleting transportation...", color: "primary"})
    const res = await ActivitiesAPI.delete(transportation_id)

    if(res.error) {
      addToast({title: "Couldn't delete transportation!", color: "danger"})
    }else {
      const updated_transportation_list = transportationList.filter((item => item.id !== transportation_id))
      setTransportationList(updated_transportation_list)
      addToast({title: "Transportation deleted successfully.", color: "success"})
    }
  }

  const tabContent = {
    notes: transportation.notes || ["No notes added yet."],
    review: transportation.review || "No review added yet.",
  };
    ...
    <div className="rounded-2xl w-full space-y-4 py-4 overflow-clip bg-brand-light border border-black/50">
        ...
        <div className="flex items-center justify-between px-4">
          <div className="flex gap-4 items-end">
              ...
              <div>
                  <h1 className="">{transportation.company}</h1>
                  <p className="text-xs">{transportation.location}</p>
              </div>
          </div>

          {user?.id == transportation.user_id && 
            <div className="flex items-center justify-center gap-5">
              ...
              <svg onClick={() => handleDelete(transportation.id)} className="size-5" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="flex items-center gap-4 px-4 py-3 justify-between">
          <div className="w-[40%]">
            <p className="text-xs text-black/70 font-medium">From</p>
            <h1 className="text-lg font-medium text-black/75">{transportation.from_state}</h1>
            <h2 className="text-sm font-medium">{transportation.from_city}</h2>
          </div>
          <div className="flex items-center justify-center w-[15%]">
            <svg className="text-brand" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 13V21M12 3V6M18.172 6C18.7024 6.00011 19.211 6.2109 19.586 6.586L21.646 8.646C21.7582 8.7581 21.8473 8.89122 21.908 9.03776C21.9688 9.1843 22 9.34137 22 9.5C22 9.65863 21.9688 9.8157 21.908 9.96224C21.8473 10.1088 21.7582 10.2419 21.646 10.354L19.586 12.414C19.211 12.7891 18.7024 12.9999 18.172 13H4C3.73478 13 3.48043 12.8946 3.29289 12.7071C3.10536 12.5196 3 12.2652 3 12V7C3 6.73478 3.10536 6.48043 3.29289 6.29289C3.48043 6.10536 3.73478 6 4 6H18.172Z" stroke="#E03E1A" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </div>
          <div className="w-[40%] text-right">
            <p className="text-xs text-black/70 font-medium">To</p>
            <h1 className="text-lg font-medium text-black/75">{transportation.to_state}</h1>
            <h2 className="text-sm font-medium">{transportation.to_city}</h2>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 px-4">
            ...
            <h4 className="text-sm font-medium">{transportation.landmark}</h4>
        </div>
        ...
        <div className="flex items-center justify-between px-4">
            <div className="flex gap-2 items-center text-sm">
                ...
                <h4>{transportation.price ? `${transportation.price} ${transportation.currency_code} ${transportation.duration}` : "N/A"}</h4>
            </div>
            <button 
              onClick={() => {
                const link = transportation.booking_link?.trim();
                if (!link) return;

                window.open(link, "_blank", "noopener,noreferrer");
              }}

              disabled={transportation.booking_link.trim() == ""} className="flex gap-2 disabled:opacity-25 items-center text-sm">
            ...
            <div className="flex gap-2 items-center text-sm">
                ...
                <h4>{transportation.rating || "N/A"} stars</h4>
            </div>
        </div>
