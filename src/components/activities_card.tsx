...
import { useActivityStore, useUserStore } from "../utils/store/app_store";
import { ActivitiesAPI } from "../utils/api";
import { addToast } from "@heroui/toast";

export default function ActivitiesCard({ activity }: { activity: any }) {
  const [activeTab, setActiveTab] = useState("notes");
  const navigate = useNavigate();
  const { user } = useUserStore();
  const { user_id } = useParams();
  const { populateActivityForm, setIsUpdating, setCurrentActivityId, setActivityList, activityList } = useActivityStore();

  const handleEdit = () => {
    populateActivityForm(activity);
    setCurrentActivityId(activity.id);
    setIsUpdating(true);
    navigate(`/${user_id}/${activity.overview_id}/new-activity`);
  };

  const handleDelete = async (activity_id: string) => {
    addToast({title: "Deleting activity...", color: "primary"})
    const res = await ActivitiesAPI.delete(activity_id)

    if(res.error) {
      addToast({title: "Couldn't delete activity!", color: "danger"})
    }else {
      const updated_activity_list = activityList.filter((item => item.id !== activity_id))
      setActivityList(updated_activity_list)
      addToast({title: "Activity deleted successfully.", color: "success"})
    }
  }

  const tabContent = {
    notes: activity.notes || ["No notes added yet."],
    review: activity.review || "No review added yet.",
  };
  ...
  return (
    <div className="rounded-2xl w-full space-y-4 py-4 overflow-clip bg-brand-light border border-black/50">
        ...
        <div className="flex items-center justify-between px-4">
          ...
          {user?.id == activity.user_id && 
            <div className="flex items-center justify-center gap-5">
              ...
              <svg onClick={() => handleDelete(activity.id)} className="size-5" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="h-40 w-full object-cover bg-gray-200">
          {activity.image ? <img src={activity.image} alt="Activity Image" className="h-full w-full object-cover" /> : <div className="h-full w-full flex items-center justify-center text-gray-400">No Image</div>}
        </div>
        ...
        <div className="flex items-center gap-2 px-4">
            ...
            <h4 className="text-sm font-medium">{activity.landmark}</h4>
        </div>
        ...
        <div className="flex items-center justify-between px-4">
            <div className="flex gap-2 items-center text-sm">
                ...
                <h4>{activity.price ? `${activity.price} ${activity.currency_code} ${activity.duration}` : "N/A"}</h4>
            </div>
            <button 
              onClick={() => {
                const link = activity.booking_link?.trim();
                if (!link) return;

                window.open(link, "_blank", "noopener,noreferrer");
              }}

              disabled={activity.booking_link.trim() == ""} className="flex gap-2 disabled:opacity-25 items-center text-sm">
            ...
            <div className="flex gap-2 items-center text-sm">
                ...
                <h4>{activity.rating || "N/A"} stars</h4>
            </div>
        </div>
