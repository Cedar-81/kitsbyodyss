...
import { useAccommodationStore, useUserStore } from "../utils/store/app_store";
...
export default function AccommodationCard({ accommodation }: { accommodation: any }) {
  ...
  const { populateAccommodationForm, setIsUpdating, setCurrentAccommodationId, setAccommodationList, accommodationList } = useAccommodationStore();
  ...
  const handleEdit = () => {
    populateAccommodationForm(accommodation);
    setCurrentAccommodationId(accommodation.id);
    setIsUpdating(true);
    navigate(`/${user_id}/${accommodation.overview_id}/new-accommodation`);
  };

  const handleDelete = async (accommodation_id: string) => {
    addToast({title: "Deleting accommodation...", color: "primary"})
    const res = await AccommodationAPI.delete(accommodation_id)

    if(res.error) {
      addToast({title: "Couldn't delete accommodation!", color: "danger"})
    }else {
      const updated_accommodation_list = accommodationList.filter((item => item.id !== accommodation_id))
      setAccommodationList(updated_accommodation_list)
      addToast({title: "Accommodation deleted successfully.", color: "success"})
    }
  }

  const tabContent = {
    notes: accommodation.notes || ["No notes yet."],
    review: accommodation.review || "No review yet.",
  };
    ...
    <div className="rounded-2xl w-full space-y-4 py-4 overflow-clip bg-brand-light border border-black/50">
        ...
        <div className="flex items-center justify-between px-4">
          <div className="flex gap-4 items-end">
              ...
              <div>
                  <h1 className="">{accommodation.name}</h1>
                  <p className="text-xs">{accommodation.location}</p>
              </div>
          </div>

          {user?.id == accommodation.user_id && 
            <div className="flex items-center justify-center gap-5">
              ...
              <svg onClick={() => handleDelete(accommodation.id)} className="size-5" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="h-40 w-full object-cover bg-gray-200">
          {accommodation.image ? <img src={accommodation.image} alt="Accommodation Image" className="h-full w-full object-cover" /> : <div className="h-full w-full flex items-center justify-center text-gray-400">No Image</div>}
        </div>
        ...
        <div className="flex items-center gap-2 px-4">
            ...
            <h4 className="text-sm font-medium">{accommodation.landmark}</h4>
        </div>

        <div className="flex items-center justify-between px-4">
            <div className="flex gap-2 items-center text-sm">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M5.83332 15V5.87667C5.83326 5.68277 5.89761 5.49435 6.01626 5.34099C6.1349 5.18763 6.30112 5.07802 6.48882 5.02938C6.67652 4.98074 6.87505 4.99582 7.05325 5.07226C7.23144 5.14869 7.3792 5.28215 7.47332 5.45167L12.5267 14.5483C12.6208 14.7179 12.7685 14.8513 12.9467 14.9277C13.1249 15.0042 13.3235 15.0193 13.5112 14.9706C13.6989 14.922 13.8651 14.8124 13.9837 14.659C14.1024 14.5057 14.1667 14.3172 14.1667 14.1233V5M4.16666 8.33333H15.8333M4.16666 11.6667H15.8333" stroke="#E03E1A" stroke-opacity="0.85" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
                <h4>{accommodation.price}{accommodation.currency_code}{accommodation.duration}</h4>
            </div>
            <button 
              onClick={() => {
                const link = accommodation.booking_link?.trim();
                if (!link) return;

                window.open(link, "_blank", "noopener,noreferrer");
              }}

              disabled={accommodation.booking_link.trim() == ""} className="flex gap-2 disabled:opacity-25 items-center text-sm">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="flex gap-2 items-center text-sm">
                ...
                <h4>{accommodation.rating || "N/A"} stars</h4>
            </div>
        </div>
