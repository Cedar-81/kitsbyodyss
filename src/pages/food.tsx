...
import { FoodAPI, OverviewAPI } from "../utils/api";
...
export default function Food() {
    ...
    // Fetch overview owner once id is available and check authorization
    useEffect(() => {
        async function fetchOverviewOwner() {
            ...
            if (res.data) {
                ...
                if (!routeMatchesOwner) {
                    if (canView) {
                        navigate(`/${kitOwnerId}/${id}/food`, { replace: true });
                        return;
                    }
                    setAuthorized(false);
                    setLoading(false);
    ...
    <div className="w-full p-5 md:px-12 space-y-4">
        ...
        {foodList.length !== 0 ? <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-6">
            {foodList.map((food) => (
