...
import ActivitiesCard from "../components/activities_card";
...
export default function Activities() {
    ...
    useEffect(() => {
        ...
        const fetchActivities = async () => {
            try {
                const res = await ActivitiesAPI.getByOverview(id!);
                setActivitiesList(res.data || []);
                setLoading(false);
                console.log("activities: ", res);
            } catch (err) {
                addToast({title: "Failed to fetch activities!", color: "danger" });
                setLoading(false);
    ...
    const handleCreate = () => {
        ...
        navigate(`/${user_id}/${id}/new-activity`);
    }
    ...
    const canCreate = profile && activitiesList.length > 0 
        ? activitiesList[0].user_id === profile.user_id // or kit.user_id
        : false;
  ...
  return (
    <div className="w-full p-5 md:px-12 space-y-4">
        ...
        {activitiesList.length !== 0 ? <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            <div className="h-[70%] w-full flex justify-center items-center">
              <div className="text-center space-y-2 w-[80%] mx-auto text-brand/40">
                ...
                <h1 className="lg:text-lg font-semibold">Woah... such emptiness! Click the + button above to create your first activity item.</h1>
              </div>
            </div>
        }
    </div>
